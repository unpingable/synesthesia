...
fn main() {
    eprintln!("synesthesia-tcp-collector supports only Linux x86_64");
    std::process::exit(2);
}
...
fn main() -> anyhow::Result<()> {
    ...
    use synesthesia::source::{tcp_ipc::TcpAggregator, tcp_live::LiveTcp};

    const WINDOW: Duration = Duration::from_millis(33);
    const MAX_RAW_PER_POLL: usize = 4_096;

    let mut capture = LiveTcp::attach()?;
